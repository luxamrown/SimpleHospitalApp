...
import (
	...
	"enigmacamp.com/rumah_sakit/manager"
...
func (c Config) ReadConfigFile(path, name string) Config {
	v := viper.New()
	v.AutomaticEnv()
...
func NewConfig(path, configFileName string) Config {
	...
	dataSourceName := fmt.Sprintf(fmt.Sprintf("postgres://%s:%s@%s:%s/%s", cfg.DbConfig.User, cfg.DbConfig.Password, cfg.DbConfig.Host, cfg.DbConfig.Port, cfg.DbConfig.Name))
